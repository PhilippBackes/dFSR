function [ theWBRawImage, theASNOut] = doWBRaw( theRawImage, theASN, theCFAPattern, theCFAMask)
...
if isempty( theASN) && ~strcmp(theCFAPattern,'random')
...
elseif isempty( theASN) && strcmp(theCFAPattern,'random')
    	% WB-Faktoren bestimmen
	myRMean = mean2( theRawImage .* single( RMask)) * 4;
	myGMean = mean2( theRawImage .* single( GMask)) * 2;
	myBMean = mean2( theRawImage .* single( BMask)) * 4;
